...
export default function Orders(
  ...
  function getOrder(orderId: number): Promise<Order> {
    return fetch(`${host}/v3.0/orders/${orderId}`, {
      method: "GET",
      headers: DEFAULT_HEADERS,
    }).then((res) => {
      if (res.status === 200) {
  ...
  function getOrders(limit: number = 100, start: number = 0): Promise<Order[]> {
    ...
    return fetch(`${host}/v3.0/orders?limit=${limit}&start=${start}`, {
      method: "GET",
      headers: DEFAULT_HEADERS,
    }).then((res) => {
      if (res.status === 200) {
  ...
  function validateOrder(orderId: number): Promise<SubmissionStatus> {
    return fetch(`${host}/v3.0/orders/${orderId}/SubmissionStatus`, {
      method: "GET",
      headers: DEFAULT_HEADERS,
    }).then((res) => {
      if (res.status === 200) {
  ...
  function updateOrderStatus(orderId: number, orderStatus: OrderStatus) {
    return fetch(`${host}/v3.0/orders/${orderId}/status`, {
      method: "POST",
      headers: DEFAULT_HEADERS,
